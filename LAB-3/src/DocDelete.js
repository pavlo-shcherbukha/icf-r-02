/**
  ...
  * @see https://cloud.ibm.com/apidocs/cloudant?code=node#deletedocument
...
function main(params) {
    ...
    return deleteDoc(service, params.dbname, params.docref.id, params.docref.rev  )
    .then( body => {
        console.log('Результат видалення документа: ');
        console.log(  body);    
        console.log( 'Формую відповідь' );
        return Promise.resolve( { ok: body.ok , dbname: params.dbname, id: body.id, rev: body.rev } );        
    })
    .catch (err => { 
...
function deleteDoc(cloudant, dbName, docid, docrev) {

    return new Promise(function (resolve, reject) {
        var l_cmd = { db: dbName,  docId: docid, rev: docrev };
        cloudant.deleteDocument( l_cmd )
        .then ( resp => {
